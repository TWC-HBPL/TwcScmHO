page 50058 "RSTN Tranfer Order"
{
    Caption = 'RSTN Out Transfer Order';
    PageType = Document;
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                {
                    ApplicationArea = Location;
                    //Editable = false;// EnableTransferFields; //PT-FBTS 25-07-24
        ...
        area(processing)
        {
            ...
            group(Release)
            {
                ...
                action("Reo&pen")
                {
                    ...
                    begin
                        IF Rec.Status = Rec.Status::Released then //PT-FBTS 25-07-24
                            Error('Once the RSTN is created, released or shipped status cant be changed or open at store level');
